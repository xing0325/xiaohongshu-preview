...
**给 AI agent 一段项目背景，它产出整套图 + 文 + 标签；你在浏览器预览，一条命令导出发布包，再整理到小红书草稿或直接发布。**

从「做完一件事」到「这件事变成一篇能发布的小红书」：图文生成、真实预览、PNG 导出、opencli 草稿发布，一条线接住。
...
这个 repo 是一条**从"项目"到"小红书草稿"的发布前流水线**：

1. **AI agent 出活**——你给它项目背景 + 调性偏好，它产出 9 张 HTML 配图 + 完整文案 + hashtag
2. **你本地预览**——打开 `index.html`，按小红书 App / 网页版的真实样子看效果
3. **导出发布包**——一条命令导出 1242×1656 PNG、标题、正文、话题和 `publish.json`
4. **整理到草稿 / 直接发布**——通过 opencli 自动上传到小红书创作者中心；默认建议先存草稿，确认后再发
...
请用下面 4 个角色帮我做一组完整的小红书图文。
产出按这个 repo 的 cards/ + cards.config.js 的结构给我；如果你能操作本机，也请继续按仓库里的命令导出发布包，并整理到小红书草稿。
...
3. 媒体知识助手 — 保证封面够钩、内页节奏、CTA 落地
   符合小红书传播逻辑，但不要油腻
4. 协助部署保姆 — 如果项目有仓库和其他资料，先检验现有资料是否足以
   支撑观众部署到自己的电脑上，然后写清楚部署步骤
...
# 发布动作
- 默认：运行 `npm run export`，然后 `npm run xhs:draft` 整理到小红书草稿
- 如果我明确说“直接发布”：运行 `npm run export`，然后 `npm run xhs:publish`
- 发布前先检查标题是否 ≤ 20 字、图片是否 ≤ 9 张、opencli 是否已登录
````
...
- 一个完整的小红书发布素材包 → `dist/xhs-pack/`
- 可选：一条已经填好的小红书草稿

打开 `index.html`，整套效果立刻可见。满意 → `npm run export` → `npm run xhs:draft` → 你确认发布。
...
你不需要每次自己打开终端。这个仓库的目标用法是：你把项目背景发给一个能操作本机的 agent，然后让它按仓库流程继续跑：

```text
请按 xiaohongshu-preview 仓库流程执行：
1. 生成 / 更新 cards/ 和 cards.config.js
2. 运行 npm run export
3. 默认运行 npm run xhs:draft 整理到小红书草稿
4. 只有我明确说“直接发布”时，才运行 npm run xhs:publish
5. 如果 opencli 未登录或 Browser Bridge 未连接，停下来让我扫码/修复，不要强行发布
```

也就是说：你负责给项目背景和最后确认；agent 负责写卡片、导出发布包、调用 opencli 上传到草稿。
